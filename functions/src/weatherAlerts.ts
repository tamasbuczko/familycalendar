 ...
 */
export const checkWeatherAndSendAlerts = async (context: functions.EventContext) => {
  try {
 ...
 */
async function getUserFamilyLocation(userId: string): Promise<string | null> {
  try {
    ...
    const familyId = userData?.currentFamilyId;
    ...
    return familyData?.location || 'Budapest,HU'; // Alapértelmezett helyszín
...
async function getWeatherData(location: string): Promise<any | null> {
  try {
    ...
    const apiKey = functions.config().openweathermap?.key || 'YOUR_API_KEY';
 ...
 */
function checkWeatherAlerts(weatherData: any, preferences: any): any[] {
  const alerts: any[] = [];
 ...
 */
async function scheduleWeatherAlert(userId: string, alert: any, weatherData: any): Promise<void> {
  try {
 ...
 * Időjárás cache frissítése
 */
async function updateWeatherCache(location: string, weatherData: any): Promise<void> {
  try {
