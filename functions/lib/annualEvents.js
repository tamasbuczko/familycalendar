...
exports.syncAnnualEvents = exports.generateBirthdayEventsForMember = exports.generateAnnualEventsForEvent = void 0;
 ...
 */
function getEventDateForYear(month, day, year) {
 ...
 */
async function isFamilyPremium(familyId, projectId) {
    try {
        ...
        if (familyDoc.exists) {
            ...
            if ((familyData === null || familyData === void 0 ? void 0 : familyData.isPremium) === true) {
                return true;
            }
        }
        // Fallback: user szintű prémium (első admin user)
        const familyData = familyDoc.data();
        if (familyData === null || familyData === void 0 ? void 0 : familyData.admin) {
            const userDoc = await admin.firestore()
                .doc(`artifacts/${projectId}/users/${familyData.admin}`)
                .get();
            if (userDoc.exists) {
                const userData = userDoc.data();
                return (userData === null || userData === void 0 ? void 0 : userData.isPremium) === true;
            }
        }
        return false;
    }
 ...
 */
async function generateEventForYear(familyId, projectId, annualEventId, year, annualEventData, isReminder = false, reminderDaysBefore = null, reminderFor = null) {
    try {
 ...
 */
async function generateAnnualEventsForEvent(familyId, projectId, annualEventId, annualEventData, isPremium) {
    try {
        const currentYear = new Date().getFullYear();
        const nextYear = currentYear + 1;
        // Fő esemény generálása jelenlegi évre és következő évre
        const mainEventCurrentYear = await generateEventForYear(familyId, projectId, annualEventId, currentYear, annualEventData, false);
        const mainEventNextYear = await generateEventForYear(familyId, projectId, annualEventId, nextYear, annualEventData, false);
        // Prémium esetén emlékeztetők generálása
        if (isPremium && annualEventData.notifyPrior !== false) {
            // 14 napos emlékeztető
            await generateEventForYear(familyId, projectId, annualEventId, currentYear, annualEventData, true, 14, mainEventCurrentYear || undefined);
            await generateEventForYear(familyId, projectId, annualEventId, nextYear, annualEventData, true, 14, mainEventNextYear || undefined);
            // 2 napos emlékeztető
            await generateEventForYear(familyId, projectId, annualEventId, currentYear, annualEventData, true, 2, mainEventCurrentYear || undefined);
            await generateEventForYear(familyId, projectId, annualEventId, nextYear, annualEventData, true, 2, mainEventNextYear || undefined);
        }
...
exports.generateAnnualEventsForEvent = generateAnnualEventsForEvent;
/**
 * Családtag születésnap események generálása
 */
async function generateBirthdayEventsForMember(familyId, projectId, memberId, memberData, isPremium) {
    try {
        ...
        const annualEventData = {
            ...
            type: 'birthday',
            date: `${birthMonth.toString().padStart(2, '0')}-${birthDay.toString().padStart(2, '0')}`,
            notifyPrior: true,
        ...
        // Generálás
        await generateAnnualEventsForEvent(familyId, projectId, annualEventId, annualEventData, isPremium);
...
exports.generateBirthdayEventsForMember = generateBirthdayEventsForMember;
/**
 * Szinkronizálás: minden annualEvent-hez generál eseményeket
 */
async function syncAnnualEvents(projectId) {
    try {
        ...
        for (const familyDoc of familiesSnapshot.docs) {
            ...
            for (const memberDoc of membersSnapshot.docs) {
                ...
                // Generálás
                await generateBirthdayEventsForMember(familyId, projectId, memberId, memberData, isPremium);
                processedCount++;
                if (processedCount >= batchSize) {
...
exports.syncAnnualEvents = syncAnnualEvents;
//# sourceMappingURL=annualEvents.js.map